...
if (${?gip_root} == 0) then                             # gip_root not defined
        ...
        set fields = `awk -F: '{{if(NR==1)nf=0;}{if((nf==0&&($1=="'${client}'"))
||(nf>0&&nf<6)){if($NF=="\"){iend=NF;}else{iend=NF+1;}for(i=1;i<iend&&nf<7;i++)
{printf("%s",$i);nf++;if(nf!=6)printf(" ");}}}}' ${gip_root}/loc/clients`
        ...
        if (${nf} != 6) then
                ...
                set fields = `awk -F: '{{if(NR==1)nf=0;}{if((nf==0&&($1=="'${sys
info}'"))||(nf>0&&nf<6)){if($NF=="\"){iend=NF;}else{iend=NF+1;}for(i=1;i<iend&&
nf<7;i++){printf("%s",$i);nf++;if(nf!=6)printf(" ");}}}}' ${gip_root}/loc/client
s`
                @ nf = ${#fields}
        endif
        if (${nf} == 6) then                            # okay
                ...
                setenv  gip_tsk ${gip_root}/tsk         # sources of application
s
                setenv  gip_tmp ${fields[6]}            # temporary files
                alias   check   ${gip_sys}/check.csh    # checks for compilation
 errors
...
#       else                                            # } else {
#               echo "GIPSY client ${client} unknown\!\!"       # error message
(suppressed)
        endif                                           # }
        unset   client                                  # remove
        unset   fields                                  # remove
...
#       line=`awk -F: '{if(substr($0,1,1)!="#")if(substr($0,length($0),1)=="\\
"){printf("%s",substr($0,1,length($0)-1));}else{printf("%s
",$0);}}' ${gip_root
}/loc/clients | grep ${client}`
